function plotting(A, x, x2, b, t1, t2)
    ...
    % We compute the norm of the solution given by the standard lsqr
    standard_norm = norm(A*x2-b)/b_norm;
    
    % We plot our solution where the x axis is the relative residual and
    % the y axis is the computation time
    scatter(normQR, t1, 50, 'filled', 'green');
    
    hold on;
    
    % We plot standard solution where the x axis is the relative residual 
    % and the y axis is the computation time
    ...
    % We set the values on the x axis from 0 to 0.1
    xlim([0,0.1])
    
    % We print the legend of the plot
    legend('Our algorithm', 'Standard LSQR');
    
    % We ceil the value of the norms
